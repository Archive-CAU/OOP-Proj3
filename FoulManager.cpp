...
{
	CCueSphere& cueSphere = dynamic_cast<CCueSphere&>(*g_sphere[0]);
	if (cueSphere.getFirstHitBallType() == BallType::NONE && status.getTurnProgressStatus() &&
		(abs(cueSphere.getVelocity_X()) < 0.01 && abs(cueSphere.getVelocity_Z()) < 0.01))
	{
		status.setFoulStatus(true);
	}
}

void FoulManager::isCueBallInHole()
{
...
{
	BallType nowBallType = dynamic_cast<CCueSphere&>(*g_sphere[0]).getFirstHitBallType();
